...
[NOTE]
====
We use the Software Package Data Exchange(R) (https://spdx.org[SPDX](R)) specification for communicating the licenses associated with software packages.
====
...
The declared licenses statement should in most cases, be exactly the same as the license statement in the file <<ip-copyright-headers,copyright and license headers>>.

Copyright holders are expressed as multiple complete copyright statements; with one complete entry on each line. In many cases, the copyright holder is different from the author (the copyright holder is very often the author's employer). When possible, the actual copyright holder should be listed. If, however, the actual copyright holder is difficult or impossible to determine, the identity of the author is sufficient for legal purposes.
...
If you have any questions or concerns, please mailto:{legalEmail}[contact] the staff of the Eclipse Foundation.