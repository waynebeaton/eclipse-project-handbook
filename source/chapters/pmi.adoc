...
A project lead or committer can create a new release by clicking menu:[Create a new release] under menu:[Committer Commands] on the project page. This opens a dialog requesting that a date and name be specified. Both of these values can be changed later. 
...
Projects should generally include more than one milestone build with each release. To include additional milestones, click the menu:[Add another item] button. Note that milestones can be dragged into the desired order. To remove a milestone, leave the _Name_ field blank.
...
If a release requires a review, you can schedule one by clicking the menu:[Schedule a review] button. The drop-down list above the button contains  several options for review dates. Pick the one that works best for you.
...
Projects cannot add themselves directly to a simultaneous release (e.g. https://projects.eclipse.org/releases/luna[Luna]), but rather must be added by the EMO (there is a https://bugs.eclipse.org/bugs/show_bug.cgi?id=402190[bug open] to extend this ability to planning council members).
...
2.  Send a note to the planning council (Eclipse projects normally do this via the https://dev.eclipse.org/mailman/listinfo/cross-project-issues-dev[cross-project-issues-dev mailing list]) with the name of your project, the name/number of the release, and the offset.
